...
\node[pb] at (2.9,4.1) {};
\node[pb] at (2.4,4.0) {};
\node[font=\scriptsize\bfseries] at (1.5,4.5) {Ausgangsraum};
...
% Zielraum (2D)
\node[font=\scriptsize\bfseries] at (1.5,1.4) {Zielraum};
...
\node[ExampleTitle, font=\scriptsize, rotate=62] at (0.8,3.3) {Abstand};
...
\foreach \t in {b1,b2,b3,b4,m1,m2,m3,m4}
...
\begin{columns}
\column{0.55\textwidth}
Mit unseren Vorbetrachtungen lässt sich die Ähnlichkeit von $\mathbf{x}_j$ zu $\mathbf{x}_i$ wird als \textbf{bedingte
Wahrscheinlichkeit} sie folgt definieren:
\[
  p_{j|i} =
