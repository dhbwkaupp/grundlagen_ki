\makesectionpage[figures/UnüberwachtesLernen.png]{Unüberwachtes\\Lernen}
...
\begin{block}{\footnotesize Kernidee}
...
\begin{itemize}
  ...
  \item Die Abstände lassen sich damit darstellen als
        $\sum_{i<j} w_{ij}\, d_{ij}^2(X) = \operatorname{tr}(X^{\mathsf{T}} V X)$.
...
\[
  \begin{aligned}
    \operatorname{tr}(X^{\mathsf{T}} V X)
...
\[
  ...
  + \underbrace{\operatorname{tr}(X^{\mathsf{T}} V X)}_{\text{quadratisch}}
\]

\column{0.40\textwidth}
\begin{block}{\footnotesize Die drei Terme}
\begin{itemize}
  \item $\sum_{i<j} w_{ij}\,\delta_{ij}^2$: nur \textbf{Abstände im Ausgangsraum},
        \textbf{konstant}.
  \item $\sum_{i<j} w_{ij}\,\delta_{ij}\,d_{ij}(X)$: enthält $d_{ij}$,
        \textbf{nicht konvex}.
  \item $\sum_{i<j} w_{ij}\,d_{ij}^2(X) = \operatorname{tr}(X^{\mathsf{T}} V X)$:
        \textbf{quadratisch}.
\end{itemize}
\end{block}
...
beschränkt:
\[
  \rho(X) \geq \operatorname{tr}\bigl(X^{\mathsf{T}} B(Z)\,Z\bigr)
\]
...
\column{0.40\textwidth}
\begin{block}{\footnotesize Perplexität}
...
\end{columns}
\source{Lösung: LösungTSneIris.ipynb}
\end{frame}
\makequestionspage{\FRAGENIMAGE}


\makesubsectionpage{Clustering}

\begin{frame}{Clustering: Grundidee}
\begin{columns}
\column{0.6\textwidth}
\begin{itemize}
  \item Bei der \textbf{Klassifikation} waren die Daten \textbf{gelabelt}.
        Die Aufteilung war dadurch vorgegeben.
  \item Beim \textbf{unüberwachten Lernen} arbeitet man mit
        \textbf{ungelabelten} Daten. Es liegt nur eine große Menge von
        Eingangspunkten vor.
  \item Ziel des \textbf{Clustering} ist es,
        \textbf{Ähnlichkeitsstrukturen} (Cluster) in der Datenmenge zu
        finden.
  \item Punkte eines Clusters sind \textbf{ähnlich} zueinander und
        \textbf{unähnlich} zu Punkten anderer Cluster.
\end{itemize}
...
\resizebox{0.9\linewidth}{!}{%
\begin{tikzpicture}[
  pt/.style={circle, fill=SecondaryColor, draw=SecondaryTitle,
             very thick, minimum size=6pt, inner sep=0pt}
]
\draw[thick, SecondaryTitle] (0,4.2) -- (0,0) -- (4.5,0);
% Cluster 1 (oben links)
\foreach \x/\y in {0.8/3.4, 1.1/3.7, 1.3/3.1, 0.9/3.0, 1.4/3.5}
  \node[pt] at (\x,\y) {};
% Cluster 2 (rechts)
\foreach \x/\y in {3.0/2.5, 3.1/2.9, 3.4/2.8, 3.6/3.2, 3.8/2.6, 3.3/2.4, 3.9/3.0}
  \node[pt] at (\x,\y) {};
% Cluster 3 (unten)
\foreach \x/\y in {1.6/1.0, 1.9/1.3, 2.1/0.8, 1.7/0.6, 2.2/1.2, 2.4/1.0}
  \node[pt] at (\x,\y) {};
\end{tikzpicture}}

{\scriptsize Ungelabelte Punkte, drei erkennbare Cluster}
\end{columns}
\end{frame}


\begin{frame}{k-Means: Grundidee}
\begin{columns}
\column{0.55\textwidth}
\begin{itemize}
  \item \textbf{k-Means} identifiziert in einer Datenmenge eine
        \textbf{vorgegebene} Anzahl $k$ von Clustern.
  \item Jeder Cluster wird durch sein \textbf{Zentrum} (Centroid)
        repräsentiert.
  \item Jeder Punkt gehört zum \textbf{nächstgelegenen} Zentrum.
  \item Das Verfahren ist \textbf{robust}, \textbf{schnell} und hat geringen
        \textbf{Speicherbedarf}.
\end{itemize}

\column{0.40\textwidth}
\begin{block}{Big Data}
Wegen dieser Eigenschaften wird k-Means sehr oft für Analysen im
\textbf{Big-Data}-Umfeld verwendet.
\end{block}
\end{columns}
\end{frame}

\begin{frame}{k-Means: Ablauf des Verfahrens}
\begin{columns}
\column{0.55\textwidth}
Das Verfahren wechselt zwischen zwei Schritten:
\begin{enumerate}[1.]
  \item \textbf{Zuordnung:} Jeder Punkt wird dem \textbf{nächstgelegenen}
        Zentrum zugeordnet.
  \item \textbf{Aktualisierung:} Jedes Zentrum wird auf den
        \textbf{Mittelwert} seiner zugeordneten Punkte gesetzt.
\end{enumerate}

\vspace{0.4em}
Beide Schritte werden \textbf{wiederholt}, bis sich die Zuordnung nicht mehr
ändert.

\column{0.40\textwidth}
\begin{blockC1}{Start}
Die $k$ Zentren werden anfangs \textbf{zufällig} gewählt, etwa als $k$
zufällige Datenpunkte.
\end{blockC1}
\end{columns}
\end{frame}

\begin{frame}{k-Means: Zielfunktion}
\begin{columns}
\column{0.55\textwidth}
k-Means minimiert die \textbf{Summe der quadrierten Abstände} jedes Punktes
zu seinem Clusterzentrum:
\[
...
\begin{itemize}
  \item $C_k$: Menge der Punkte im Cluster $k$.
  \item $\boldsymbol{\mu}_k$: Zentrum des Clusters $k$.
  \item Diese Größe heißt auch \textbf{Within-Cluster Sum of Squares}
        (WCSS).
\end{itemize}

\column{0.40\textwidth}
\begin{blockC2}{Wirkung}
Kleine $J$ bedeuten \textbf{kompakte} Cluster: die Punkte liegen nah an
ihrem Zentrum.
\end{blockC2}
\end{columns}
\end{frame}
...
\begin{frame}{k-Means: Ablauf}

\begin{columns}

\column{0.48\textwidth}
\begin{itemize}
  \item<1-> Start mit \textbf{zufälligen Zentren}
  \item<2-> Punkte werden \textbf{zugeordnet}
  \item<3-> Zentren \textbf{verschieben sich}
  \item<4-> \textbf{Neuzuordnung:} Ein Punkt \textbf{wechselt Cluster}
  \item<5-> Zentren \textbf{verschieben sich} und werden \textbf{stabil}
\end{itemize}
...
\begin{tikzpicture}[
  pg/.style={circle, fill=gray!40, draw=gray,
             very thick, minimum size=6pt, inner sep=0pt},
  pa/.style={circle, fill=SecondaryColor, draw=SecondaryTitle,
             very thick, minimum size=6pt, inner sep=0pt},
  pb/.style={circle, fill=AccentColor, draw=AccentTitle,
...
% Rahmen
\draw[very thick, PrimaryTitle] (0.7,0) rectangle (5.5,3.9);

% -------------------------------
% Punkte (fixe Positionen)
% -------------------------------

% Overlay 1: alle grau
\only<1>{
  \foreach \x/\y in {1.0/3.3, 1.3/3.6, 1.5/3.1, 1.1/2.8,
                    3.7/1.2, 4.3/1.5, 4.4/0.9, 3.9/0.8,
                    3.0/2.0}
    \node[pg] at (\x,\y) {};
}

% Overlay 2-3: erste Zuordnung
\only<2-3>{
  \foreach \x/\y in {1.0/3.3, 1.3/3.6, 1.5/3.1, 1.1/2.8}
    \node[pa] at (\x,\y) {};
  \foreach \x/\y in {3.7/1.2, 4.3/1.5, 4.4/0.9, 3.9/0.8}
    \node[pb] at (\x,\y) {};
  \node[pa] (switch) at (3.0,2.0) {};
}

% Overlay 4-5: Punkt wechselt Cluster
\only<4-5>{
  \foreach \x/\y in {1.0/3.3, 1.3/3.6, 1.5/3.1, 1.1/2.8}
    \node[pa] at (\x,\y) {};
  \foreach \x/\y in {3.7/1.2, 4.3/1.5, 4.4/0.9, 3.9/0.8}
    \node[pb] at (\x,\y) {};
  \node[pb] (switch) at (3.0,2.0) {};
}

% -------------------------------
% Zentren (bewegen sich)
% -------------------------------

% 1
\only<1>{
  \node[centA] at (2.5,3.2) {};
  \node[centB] at (5.0,0.3) {};
}

% 2
\only<2>{
  \node[centA] (cA) at (2.5,3.2) {};
  \node[centB] (cB) at (5.0,0.3) {};
  \draw[thin,SecondaryTitle] (cA) -- (switch);
}

% 3
\only<3>{
  \node[centA] (cA) at (1.6,2.96) {};
  \node[centB] (cB) at (4.07,1.1) {};
  \draw[thin,SecondaryTitle] (cA) -- (switch);
  }
...
\only<4>{
  \node[centA] (cA) at (1.6,2.96) {};
  \node[centB] (cB) at (4.07,1.1) {};
  \draw[thin,AccentTitle] (cB) -- (switch);
}

% 5
\only<5>{
  \node[centA] at (1.3,3.2) {};
  \node[centB] at (3.9,1.3) {};
}
...
\vspace{0.3em}
{\scriptsize Kreuze sind die Cluster-Zentren}
...
\begin{frame}{k-Means: Eigenschaften und Grenzen}
\begin{columns}
\column{0.55\textwidth}
\begin{itemize}
  \item Die Clusterzahl $k$ muss \textbf{vorab} festgelegt werden.
  \item Das Ergebnis hängt von der \textbf{Startwahl} der Zentren ab.
  \item k-Means bevorzugt \textbf{kugelförmige}, gleich große Cluster.
  \item Bei \textbf{länglichen} oder unterschiedlich dichten Clustern
        liefert es oft schlechte Ergebnisse.
\end{itemize}

\column{0.40\textwidth}
\begin{blockC2}{Praxis}
Man startet das Verfahren oft \textbf{mehrfach} mit verschiedenen
Initialisierungen und wählt das Ergebnis mit dem kleinsten $J$.
\end{blockC2}
\end{columns}
\end{frame}


\begin{frame}[fragile]{k-Means in Python: Skalierung und Clustering}
\begin{columns}
\column{0.55\textwidth}
k-Means arbeitet mit \textbf{Abständen}, daher ist eine \textbf{Skalierung}
nötig:
\begin{minted}[fontsize=\footnotesize]{python}
from sklearn.preprocessing import StandardScaler
scaler = StandardScaler()
...
\vspace{0.3em}
Anschließend wird das Modell mit $k = 3$ Clustern trainiert:
\begin{minted}[fontsize=\footnotesize]{python}
...
\texttt{n\_clusters} legt die \textbf{Anzahl} der gesuchten Cluster fest.
Sie muss vorab bekannt sein.
\end{block}
\end{columns}
\source{KMeansFahrzeugdatenMitCentroids.ipynb}
\end{frame}

\begin{frame}[fragile]{k-Means in Python: Labels und Centroiden}
\begin{columns}
\column{0.48\textwidth}
\begin{minted}[fontsize=\footnotesize]{python}
# Label vom Clusterer abfragen
labels = model.labels_

# Centroide abfragen
centers = model.cluster_centers_

# Centroide zurücktransformieren
centers_transformed = \
...
\vspace{0.3em}
\footnotesize
Die Centroiden liegen im \textbf{skalierten} Raum. Vor dem Zeichnen werden
sie \textbf{zurücktransformiert}.

\column{0.47\textwidth}
\centering
\includesvg[width=\linewidth]{figures/kmeans_fahrzeuge.svg}
\end{columns}
\source{KMeansFahrzeugdatenMitCentroids.ipynb}
\end{frame}
...
\begin{frame}{k-Means: Wahl von $k$ mit der Elbow-Methode}
\begin{columns}
\column{0.55\textwidth}
\begin{itemize}
  \item Die Clusterzahl $k$ muss vorab festgelegt werden. Oft ist sie aber
        \textbf{unbekannt}.
  \item Die \textbf{Elbow-Methode} hilft bei der Wahl eines sinnvollen $k$.
  \item Man führt k-Means für \textbf{verschiedene} $k$ aus und misst jeweils
        die Zielfunktion:
        \[
          J(k) = \sum_{i=1}^{k} \sum_{\mathbf{x} \in C_i}
                 \lVert \mathbf{x} - \boldsymbol{\mu}_i \rVert^2
        \]
  \item $J(k)$ wird gegen $k$ aufgetragen.
\end{itemize}

\column{0.40\textwidth}
\begin{block}{Grundgedanke}
Mit mehr Clustern sinkt $J$ immer weiter. Gesucht ist der Punkt, ab dem
weitere Cluster \textbf{kaum noch} etwas bringen.
\end{block}
\end{columns}
\end{frame}


\begin{frame}{k-Means: Wie viele Centroiden?}
...
\begin{scope}[yshift=0cm]
  ...
  \draw[SecondaryTitle,very thick] (0,0) rectangle (6,2.2);

  \node[font=\scriptsize] at (3,1.9) {Centroid};
...
\begin{scope}[yshift=-2.7cm]
  ...
  \foreach \x/\y in {
    ...
    0.4/0.6,
    1.6/0.6}
    \draw[thin,color=PrimaryColor] (c2a) -- (\x,\y);

  % Cluster rechts
  \node[cbl] (c2b) at (4.3,1.1) {};
  ...
  \foreach \x/\y in {
    ...
    4.7/1.5,
    4.4/0.6}
    \draw[thin,color=AlertColor] (c2b) -- (\x,\y);
...
\begin{scope}[yshift=-5.4cm]
  ...
  \foreach \x/\y in {
    0.9/1.9,
    1.7/1.7}
    \draw[thin,color=AccentColor] (c3g) -- (\x,\y);

  % Cluster unten links
  \node[cpk] (c3m) at (0.7,0.7) {};
  ...
  \foreach \x/\y in {
    0.4/1.0,
    0.9/0.4}
    \draw[thin,color=PrimaryColor] (c3m) -- (\x,\y);

  % Cluster rechts
  \node[cbl] (c3b) at (4.3,1.1) {};
  ...
  \foreach \x/\y in {
    ...
    4.7/1.5,
    4.4/0.6}
    \draw[thin,color=AlertColor] (c3b) -- (\x,\y);
...
\textbf{Zu wenig Centroiden:} die mittleren Abstände sind \textbf{groß}.

\vspace{4.em}
\textbf{Genug Centroiden:} die Abstände sind deutlich \textbf{kleiner}.

\vspace{4.em}
\textbf{Zu viele Centroiden:} die Abstände sind nur noch ein \textbf{bisschen} kleiner.
...
\begin{frame}{k-Means: Das Elbow-Diagramm}
\begin{columns}
\column{0.47\textwidth}
\begin{itemize}
  \item Anfangs fällt $J(k)$ \textbf{stark}, da jeder neue Cluster viel
        Fehler beseitigt.
  \item Ab einem bestimmten $k$ flacht die Kurve deutlich \textbf{ab}.
  \item Dieser Knick heißt \textbf{Ellbogen} (elbow).
  \item Das zugehörige $k$ ist eine gute \textbf{Wahl} für die Clusterzahl.
\end{itemize}
...
\begin{tikzpicture}[>=Stealth]
% Achsen
\draw[->, thick] (0,0) -- (5.5,0) node[right, font=\scriptsize] {$k$};
\draw[->, thick] (0,0) -- (0,4.6) node[above, font=\scriptsize] {$J(k)$};
% x-Ticks
\foreach \k/\x in {1/0.6,2/1.2,3/1.8,4/2.4,5/3.0,6/3.6,7/4.2,8/4.8}
  \draw (\x,0.08) -- (\x,-0.08) node[below, font=\scriptsize] {\k};
% Kurve (WCSS skaliert: /100 * 4)
\draw[SecondaryColor, line width=1.2pt]
  (0.6,4.0) -- (1.2,2.2) -- (1.8,1.2) -- (2.4,0.96) --
  (3.0,0.8) -- (3.6,0.68) -- (4.2,0.6) -- (4.8,0.56);
% Punkte
\foreach \x/\y in {0.6/4.0,1.2/2.2,1.8/1.2,2.4/0.96,3.0/0.8,3.6/0.68,4.2/0.6,4.8/0.56}
  \fill[SecondaryTitle] (\x,\y) circle (2pt);
% Ellbogen markieren bei k=3
\draw[AlertTitle, thick, dashed] (1.8,1.2) circle (0.35);
\node[AlertTitle, font=\scriptsize, anchor=west] at (2.2,1.9) {Ellbogen};
\end{tikzpicture}}
\end{columns}
\end{frame}
...
\begin{frame}[fragile]{k-Means: Elbow-Methode in Python}
\begin{columns}
\column{0.55\textwidth}
Für verschiedene $k$ wird die Zielfunktion (quadratische Abstände) ermittelt
und gesammelt:
\begin{minted}[fontsize=\footnotesize]{python}
from sklearn.cluster import KMeans
...
\column{0.40\textwidth}
\begin{blockC1}{\texttt{inertia\_}}
Das Attribut \texttt{inertia\_} enthält die \textbf{Summe der quadrierten
Abstände} jedes Punktes zu seinem Clusterzentrum, also die Zielfunktion
$J(k)$.
\end{blockC1}
\end{columns}
\source{KMeansIrisElbow.ipynb}
\end{frame}

\begin{frame}[fragile]{k-Means: Elbow-Graph zeichnen}
  \footnotesize
\begin{columns}
\column{0.55\textwidth}
Die gesammelten Werte werden gegen $k$ aufgetragen:
\begin{minted}[fontsize=\footnotesize]{python}
%matplotlib inline
...
\vspace{0.3em}
Der große \textbf{Knick} bei $K=3$ zeigt ein sinnvolles $k$ an.\par\medskip

\begin{blockC1}{\footnotesize Mehrere Knicke}
Oft ist der Ellbogen nicht eindeutig. Zeigt die Kurve mehrere Knicke, testet man die zugehörigen Werte für $k$ einzeln und vergleicht die entstehenden Cluster.
\end{blockC1}

\column{0.4\textwidth}
\centering
\includesvg[width=\linewidth]{figures/elbow_iris.svg}
\end{columns}
\source{KMeansIrisElbow.ipynb}
\end{frame}

\begin{frame}{k-Means: Iris-Cluster mit $k = 3$}
\begin{columns}
\column{0.47\textwidth}
\begin{itemize}
  \item Mit $k = 3$ ergeben sich drei Cluster in den Iris-Daten.
  \item Die vierdimensionalen Daten werden per \textbf{MDS} in die Ebene
        projiziert.
  \item Die von k-Means gefundenen Cluster sind \textbf{eingefärbt}.
\end{itemize}

\column{0.48\textwidth}
\centering
\includesvg[width=\linewidth]{figures/kmeans_iris.svg}
\end{columns}
\source{\cite{seemann2024mlaz}}
\end{frame}

\begin{frame}{Übung: Clustering des Heart-Disease-Datensatzes}
\footnotesize
\begin{columns}  
\column{0.60\textwidth}
Untersuchen Sie den \textbf{Heart-Disease}-Datensatz mit k-Means:

\begin{itemize}
  \item Verwenden Sie nur \textbf{folgende Spalten: } \texttt{'age', 'chol', 'num'}.
  \item Verwerfen Sie unvollständige Zeilen mit \texttt{\bfseries dropna() }
  \item Bestimmen Sie die \textbf{Clusteranzahl} mit der Elbow-Methode.
  \item Führen Sie das \textbf{Clustering} mit dem gewählten $k$ durch.
  \item Projizieren Sie die Daten mit \textbf{t-SNE} in 2D.
  \item Stellen Sie die projizierten Daten samt \textbf{Clustern} in einem
        Scatterplot dar.
\end{itemize}
...
\twemoji[width=0.9\linewidth]{laptop}
\end{columns}
\source{Lösung: LösungKMeansHeartDisease.ipynb}

\note{Alter und Cholesterinwert sind wohl die Haupteinflussfaktoren.}
