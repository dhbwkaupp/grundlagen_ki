...
\begin{frame}{Einlagiges Perzeptron: Schwellenwertfunktion}
\begin{columns}
\column{0.47\textwidth}
\begin{itemize}
  \item Die mathematische Nachbildung \textbf{feuert} ab Überschreitung
        eines \textbf{Schwellenwertes} $\theta$.
  \item Sie springt dabei von $0$ auf $1$ (\textbf{Heaviside-Funktion}).
...
% Schwellenwertfunktion mit Schwelle theta
...
\begin{itemize}
  ...
  \item Fasst man Gewichte und Eingabe als \textbf{Vektoren}
        $\mathbf{w} = (w_0, \dots, w_n)^{\mathsf{T}}$ und
        $\mathbf{x} = (x_0, \dots, x_n)^{\mathsf{T}}$ auf, wird daraus ein
        \textbf{Skalarprodukt}:
        \[
...
\begin{frame}{Perzeptron: Schwellenwert als Trenngerade}
\small
\begin{columns}
\column{0.47\textwidth}
\begin{itemize}
  \item Wir nehmen jetzt noch den den Schwellenwert $\theta$ hinzu und stellen um:
        \[
...
\begin{itemize}
  ...
  \item $g$ wird \textbf{komponentenweise} angewandt (Schwellenwert-/%
        Heaviside-Funktion).
...
\begin{tikzpicture}[line cap=round, line join=round]
    ...
    \tikzset{
        ...
        sep/.style={draw=AlertTitle, line width=1.2pt},
...
\source{GradientenabstiegBackpropagation.ipynb}
\end{frame}
...
\begin{itemize}
    ...
    \item Alle Netzfunktionen setzt man in die \textbf{Fehlerfunktion} ein.

    \item Danach bildet man die \textbf{partiellen Ableitungen
          nach allen Gewichten}.
...
\[
L=\sum_{(x_i,y_i)\in D}\Bigl(W^{(3)}\operatorname{sig}\bigl(W^{(2)}\operatorname{sig}(W^{(1)}x_i)\bigr)- y_i\Bigr)^2
\]
\end{blockC3}
...
\[
    ...
    \dots,
    \frac{\partial L}{\partial w_{m,n}^{(3)}}
\]
\end{blockC4}
...
\end{blockC1}
\end{columns}
\source{MultilayerPerceptron.ipynb}
\end{frame}
...
\end{columns}
\source{Lösung: LösungMlpIris.ipynb, LösungMlpHeartDisease.ipynb}
\end{frame}
