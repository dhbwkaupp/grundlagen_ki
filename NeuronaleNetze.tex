...
\begin{itemize}
  ...
  \item Das Ergebnis ist die \textbf{quadratische Fehlersumme} $C$.
\end{itemize}
...
\node[font=\small] at (5.0,5.4)
  {$C = \sum_{y_i \in D} (\hat{y}_i - y_i)^2$};
% Input
...
\vspace{0.2cm}
{\scriptsize Beispielhafte Funktion des Gesamtfehlers $C$ in Abhängigkeit von einem einzigen Gewicht.}
...
\begin{itemize}
    ...
    \item Dazu wird die Steigung der Fehlerfunktion ausgewertet.
...
\begin{itemize}
    ...
    \item Dazu muss die Fehlerfunktion nach \textbf{allen Gewichten}
          abgeleitet werden.

    \item Jede Ableitung beschreibt, wie stark ein Gewicht zum Fehler beiträgt.
\end{itemize}
...
\begin{itemize}
    ...
    \item Die Fehlerfunktion summiert die Abweichungen über alle
          Trainingsbeispiele.

    \item Da $\hat{y}_i$ von den Gewichten abhängt, ist auch $C$ eine Funktion
          aller Gewichte.

    \item Gesucht sind Gewichte, für die $C$ minimal wird.
\end{itemize}
...
\column{0.48\textwidth}
\footnotesize
\begin{blockC3}{\footnotesize Einsetzen in die Fehlerfunktion}
\[
\begin{aligned}
C
&=
...
\[
    ...
    \dots,
    \frac{\partial C}{\partial w_{1,4}^{(3)}},
\]
\end{blockC4}
...
\item Liefert: $\frac{\partial C}{\partial w}$
\end{itemize}
...
\item Ziel: Minimierung der Loss-Funktion
